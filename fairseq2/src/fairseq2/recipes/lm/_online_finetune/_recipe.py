...
from fairseq2.optim import ADAMW_OPTIMIZER, AdamWConfig, ParameterCollection
...
from fairseq2.recipes.lm._online_finetune._grpo_game import (
    GrpoGameFinetuneConfig,
)
...
from fairseq2.recipes.lm._online_finetune._online_dpo_game import (  # ONLINE_DPO_GAME_FINETUNE_UNIT,
    OnlineDpoGameFinetuneConfig,
)
...
class OnlineFinetuneDatasetSection(DatasetSection):
    ...
    """The splits to use for training. Splits will be cycled through during training."""

    train_skip_attacker: List[bool] | None = None
    """Whether to skip the attacker model update for the corresponding train split. Will be set to `False` if not specified."""

    train_dataset_scales: List[float] | None = None
    """The loss scale factor of each training dataset. Will be set to `1.0` if not specified."""

    valid_split: List[str] | None = None

    valid_skip_attacker: List[bool] | None = None
    ...
    def validate(self) -> None:
        ...
        if self.train_split is None or len(self.train_split) == 0:
            result.add_error("`train_split` must be a non-empty list of split names.")

        if self.train_skip_attacker is None:
            self.train_skip_attacker = [False] * len(self.train_split)

        if self.train_dataset_scales is None:
            self.train_dataset_scales = [1.0] * len(self.train_split)

        if len(self.train_split) != len(self.train_skip_attacker):
            result.add_error(
                "`train_skip_attacker` must have the same length as `train_split`."
            )

        if len(self.train_split) != len(self.train_dataset_scales):
            result.add_error(
                "`train_dataset_scales` must have the same length as `train_split`."
            )

        if self.valid_split is not None:
            if len(self.valid_split) != len(self.valid_skip_attacker):
                result.add_error(
                    "`valid_skip_attacker` must have the same length as `valid_split`."
                )

        if result.has_error:
            raise ValidationError(
                "The regime configuration section has one or more validation errors:",
                result,  # fmt: skip
            )
...
@dataclass(kw_only=True)
class OnlineFinetuneGameConfig:
    model_attacker: ModelSection = field(
        default_factory=lambda: ModelSection(
            name="llama3_1_8b_instruct_abliterated_scratch"
        )
    )

    model_defender: ModelSection = field(
        default_factory=lambda: ModelSection(name="llama3_1_8b_instruct_scratch")
    )

    ema_model_attacker: ModelSection | None = field(
        default_factory=lambda: ModelSection(
            name="llama3_1_8b_instruct_abliterated_scratch"
        )
    )

    ema_model_defender: ModelSection | None = field(
        default_factory=lambda: ModelSection(name="llama3_1_8b_instruct_scratch")
    )

    ref_model_attacker: ModelSection | None = field(
        default_factory=lambda: ModelSection(
            name="llama3_1_8b_instruct_abliterated_scratch"
        )
    )

    ref_model_defender: ModelSection | None = field(
        default_factory=lambda: ModelSection(name="llama3_1_8b_instruct_scratch")
    )
    ...
    tokenizer: TextTokenizerSection = field(
        default_factory=lambda: TextTokenizerSection(
            name="llama3_1_8b_instruct_abliterated_scratch"
        )
    )
    ...
    criterion: OnlineCriterionSection = field(
        default_factory=lambda: OnlineCriterionSection(name="dpo_game", config={})
    )

    optimizer_attacker: OptimizerSection = field(
        default_factory=lambda: OptimizerSection(
            name=ADAMW_OPTIMIZER,
            config=AdamWConfig(lr=5.5e-06, betas=(0.9, 0.95), weight_decay=0.1),
        )
    )
    optimizer_defender: OptimizerSection = field(
        default_factory=lambda: OptimizerSection(
            name=ADAMW_OPTIMIZER,
            config=AdamWConfig(lr=5.5e-06, betas=(0.9, 0.95), weight_decay=0.1),
        )
    ...
    lr_scheduler_attacker: LRSchedulerSection = field(
        default_factory=lambda: LRSchedulerSection(
            name=COSINE_ANNEALING_LR, config=CosineAnnealingLRConfig(final_lr_scale=0.2)
        )
    )
    lr_scheduler_defender: LRSchedulerSection = field(
        default_factory=lambda: LRSchedulerSection(
            name=COSINE_ANNEALING_LR, config=CosineAnnealingLRConfig(final_lr_scale=0.2)
        )
    )
...
def register_online_finetune_game_configs(context: RuntimeContext) -> None:
    registry = context.get_config_registry(OnlineFinetuneGameConfig)

    preset = registry.decorator

    @preset("llama3_1_instruct")
    def llama3_1_instruct() -> OnlineFinetuneGameConfig:
        config = OnlineFinetuneGameConfig()

        config.model_attacker.config = DropoutConfig()
        config.model_defender.config = DropoutConfig()
        config.regime.validate_before_training = True

        return config

    @preset("llama3_1_instruct_online_dpo_game")
    def llama3_1_instruct() -> OnlineFinetuneGameConfig:
        config = OnlineFinetuneGameConfig()

        config.model_attacker.config = DropoutConfig()
        config.model_defender.config = DropoutConfig()
        config.criterion.config = OnlineDpoGameFinetuneConfig()

        return config

    @preset("llama3_1_instruct_online_grpo_game")
    def llama3_1_instruct() -> OnlineFinetuneGameConfig:
        config = OnlineFinetuneGameConfig()

        config.model_attacker.config = DropoutConfig()
        config.model_defender.config = DropoutConfig()
        config.criterion.config = GrpoGameFinetuneConfig()

        return config


def load_online_finetuner_game(
    context: RuntimeContext, config: object, output_dir: Path
) -> Trainer[PreferenceBatch]:
    config = structure(config, OnlineFinetuneGameConfig)
    ...
    output_dir_attacker = output_dir.joinpath("attacker")
    output_dir_defender = output_dir.joinpath("defender")
    output_dir_ema_attacker = output_dir.joinpath("ema_attacker")
    output_dir_ema_defender = output_dir.joinpath("ema_defender")
    output_dir_ref_attacker = output_dir.joinpath("ref_attacker")
    output_dir_ref_defender = output_dir.joinpath("ref_defender")

    checkpoint_manager_attacker = create_checkpoint_manager(
        context, gangs, output_dir_attacker
    )
    checkpoint_manager_defender = create_checkpoint_manager(
        context, gangs, output_dir_defender
    )
    checkpoint_manager_ema_attacker = create_checkpoint_manager(
        context, gangs, output_dir_ema_attacker
    )
    checkpoint_manager_ema_defender = create_checkpoint_manager(
        context, gangs, output_dir_ema_defender
    )
    checkpoint_manager_ref_attacker = create_checkpoint_manager(
        context, gangs, output_dir_ref_attacker
    )
    checkpoint_manager_ref_defender = create_checkpoint_manager(
        context, gangs, output_dir_ref_defender
    )
    ...
    model_attacker = setup_model(
        ...
        output_dir_attacker,
        gangs,
        checkpoint_manager_attacker,
    )
    model_defender = setup_model(
        CausalLM,
        context,
        config.model_defender,
        config.trainer,
        output_dir_defender,
        gangs,
        checkpoint_manager_defender,
    )

    ema_model_attacker = setup_model(
        CausalLM,
        context,
        config.ema_model_attacker,
        config.trainer,
        output_dir_ema_attacker,
        gangs,
        checkpoint_manager_ema_attacker,
    )
    ema_model_defender = setup_model(
        CausalLM,
        context,
        config.ema_model_defender,
        config.trainer,
        output_dir_ema_defender,
        gangs,
        checkpoint_manager_ema_defender,
    )

    ema_model_attacker.module.eval()
    ema_model_defender.module.eval()
    freeze_parameters(ema_model_attacker.module)
    freeze_parameters(ema_model_defender.module)

    ref_model_attacker = setup_model(
        CausalLM,
        context,
        config.ref_model_attacker,
        config.trainer,
        output_dir_ref_attacker,
        gangs,
        checkpoint_manager_ref_attacker,
    )
    ref_model_defender = setup_model(
        CausalLM,
        context,
        config.ref_model_defender,
        config.trainer,
        output_dir_ref_defender,
        gangs,
        checkpoint_manager_ref_defender,
    )

    ref_model_attacker.module.eval()
    ref_model_defender.module.eval()
    freeze_parameters(ref_model_attacker.module)
    freeze_parameters(ref_model_defender.module)

    # set parameter converter to sync with vllm
    model_attacker._convert_parameter = get_parameter_converter(model_attacker.config)
    model_defender._convert_parameter = get_parameter_converter(model_defender.config)
    ema_model_attacker._convert_parameter = get_parameter_converter(
        ema_model_attacker.config
    )
    ema_model_defender._convert_parameter = get_parameter_converter(
        ema_model_defender.config
    )
    ref_model_attacker._convert_parameter = get_parameter_converter(
        ref_model_attacker.config
    )
    ref_model_defender._convert_parameter = get_parameter_converter(
        ref_model_defender.config
    )

    optimizer_attacker = create_optimizer(
        context, config.optimizer_attacker, model_attacker
    )
    optimizer_defender = create_optimizer(
        context, config.optimizer_defender, model_defender
    )

    lr_scheduler_attacker = create_lr_scheduler(
        context, config.lr_scheduler_attacker, config.regime, optimizer_attacker
    )
    lr_scheduler_defender = create_lr_scheduler(
        context, config.lr_scheduler_defender, config.regime, optimizer_defender
    )
    ...
    for actor_config in config.vllm.ray_actors:
        if (
            isinstance(model_attacker.config, QwenConfig)
            and actor_config.ray_actor_name == "vllm_model_attacker"
        ):
            actor_config.vllm_sampling_params["allowed_token_ids"] = list(
                range(vocab_size)
            )
        ...
        if (
            isinstance(model_defender.config, QwenConfig)
            and actor_config.ray_actor_name == "vllm_model_defender"
        ):
            actor_config.vllm_sampling_params["allowed_token_ids"] = list(
                range(vocab_size)
            )
    ...
    valid_unit = unit_handler.create(
        model_attacker,
        model_defender,
        ema_model_attacker,
        ema_model_defender,
    ...
    repeat_batch_n_times = 1
    grad_accumulation = config.trainer.grad_accumulation.num_batches
    if unit.display_name == "GRPO_game" or unit.display_name == "online_dpo_game":
        repeat_batch_n_times = (
            unit._config.loss_config.num_microbatches_B
            * unit._config.loss_config.num_microbatches_A
            * unit._config.loss_config.num_microbatches_D
        )
        grad_accumulation = grad_accumulation * repeat_batch_n_times
        if repeat_batch_n_times > 1:
            log.info(
                f"Using micro-batching: group_size={unit._config.loss_config.num_microbatches_B}*{unit._config.loss_config.num_microbatches_A}*{unit._config.loss_config.num_microbatches_D}, thus effective grad_accumulation={grad_accumulation}, repeat_batch_n_time={repeat_batch_n_times}"
            )
            assert (
                config.dataset.batch_size % unit._config.loss_config.num_microbatches_B
                == 0
            )
            assert (
                unit._reward._rollout_tree_config.num_attacker_samples
                % unit._config.loss_config.num_microbatches_A
                == 0
            )
            assert (
                unit._reward._rollout_tree_config.num_defender_samples
                % unit._config.loss_config.num_microbatches_D
                == 0
            )

    data_readers = []
    for train_split, skip_attacker, scale in zip(
        config.dataset.train_split,
        config.dataset.train_skip_attacker,
        config.dataset.train_dataset_scales,
    ):
        extras = deepcopy(config.dataset.extras)
        extras["skip_attacker"] = skip_attacker
        extras["dataset_scale"] = scale
        read_options = PromptReadOptions(
            batching=batching,
            example_shuffle_window=config.dataset.example_shuffle_window,
            batch_shuffle_window=config.dataset.batch_shuffle_window,
    ...
    if config.dataset.valid_split:
        if config.dataset.eval_batch_size is not None:
            valid_batching = StaticBatching(config.dataset.eval_batch_size)
        else:
            raise ValueError
        valid_data_readers = []
        for valid_split, skip_attacker in zip(
            config.dataset.valid_split, config.dataset.valid_skip_attacker
        ):
            extras = deepcopy(config.dataset.extras)
            extras["skip_attacker"] = skip_attacker
            valid_read_options = PromptReadOptions(
