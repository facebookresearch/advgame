...
class RayActorConfig(ABC):
    ...
    sync_schedule: str = "all"
    sync_round_robin_switch_every: int = 0
    sync_smart_coverage_power: float = 2.0
...
def do_sync_smart_coverage(
    ...
    """
    k = len(last_sync_at)
    assert all(isinstance(s, int) for s in last_sync_at), (
        "last_sync_at must be integers"
    )
    ...
    while True:
        ...
        viol_j = next(
            (j for j in range(1, k + 1) if sorted_ages[j - 1] > fences[j - 1]), None
        )
        if viol_j is None:
            break
...
class RemoteVllmModel:
    def __init__(
        ...
        sync_schedule: str,
        sync_round_robin_switch_every: int,
        sync_smart_coverage_power: float,
        ...
        if sync_schedule == "all":
            pass
        elif sync_schedule == "round_robin":
            assert num_replicas > 1
            self.sync_round_robin_switch_every = sync_round_robin_switch_every
        elif sync_schedule == "smart_coverage":
            assert num_replicas > 1
            self.last_sync = [0] * num_replicas
            self.sync_smart_coverage_power = sync_smart_coverage_power
        else:
            raise ValueError(f"Unknown sync schedule: {self.sync_schedule}")
    ...
    def sync_weights_with_vllm(self, model, trainer_step_nr, converter=None):
        ...
        def sync_replica_i(replica_i):
            log.info(f"Syncing replica {replica_i}")
        ...
        if self.sync_schedule == "all":
            # iterate over all replicas
            for replica_i in range(self.num_replicas):
                sync_replica_i(replica_i)
        elif self.sync_schedule == "round_robin":
            replica_i = (
                trainer_step_nr // self.sync_round_robin_switch_every
            ) % self.num_replicas
            sync_replica_i(replica_i)
        elif self.sync_schedule == "smart_coverage":
            self.last_sync, info = do_sync_smart_coverage(
                trainer_step_nr,
                self.last_sync,
                sync_fn=sync_replica_i,
                p=self.sync_smart_coverage_power,
                g=1,
                always_fresh=True,
            )
            log.info(f"Smart sync info: {info}")
        else:
            raise ValueError(f"Unknown sync schedule: {self.sync_schedule}")
    ...
    def rollout_from_model(self, prompt_list, sampling_params=None, string_input=False):
        ...
        prompt_ids = list(range(len(prompt_list)))

        if not self.sync_schedule == "all":
            # replicas are not equal, so we randomly distribute
            perm = list(range(len(prompt_list)))
            random.shuffle(perm)
            prompt_list = [prompt_list[i] for i in perm]
            prompt_ids = [prompt_ids[i] for i in perm]
        ...
        for i in range(self.num_replicas):
            ...
            chunk_ids.append(prompt_ids[start:end])
            start = end

        # Process each chunk with a different replica
        replica_ids = list(range(self.num_replicas))
        if not self.sync_schedule == "all":
            random.shuffle(replica_ids)
        outputs = []
        for replica_i, chunk, prompt_ids in zip(replica_ids, chunks, chunk_ids):
            if len(chunk) > 0:  # Only send non-empty chunks
                ...
                # log.info(f"Prompt ids {prompt_ids} sent to replica {replica_i}")
        ...
        if not self.sync_schedule == "all":
            inv_perm = [0] * len(prompt_list)
            for k, j in enumerate(perm):
                inv_perm[j] = k
            rollouts = [rollouts[i] for i in inv_perm]
        ...
        if gangs.dp.rank == 0 and gangs.tp.rank == 0:
            ...
            if actor_config.backend == "vllm":
                ...
                remote_vllm_model = RemoteVllmModel(
                    ...
                    actor_config.sync_schedule,
                    actor_config.sync_round_robin_switch_every,
                    actor_config.sync_smart_coverage_power,
                    context,
                    gangs,
                )
            else:
