...
def update_logps_metrics(
    ...
    avg_chosen_logps: Tensor = None,
    avg_rejected_logps: Tensor = None,
    ...
    metric_bag.get(Mean, prefix + "chosen_logps").update(
        chosen_logps.sum() / batch.chosen.batch_size,
        weight=batch.chosen.batch_size * weight_factor,
    )
    rejected_logps = rejected_logps.detach()
    metric_bag.get(Mean, prefix + "rejected_logps").update(
        rejected_logps.sum() / batch.rejected.batch_size,
        weight=batch.rejected.batch_size * weight_factor,
    )

    if avg_chosen_logps is not None:
        avg_chosen_logps = avg_chosen_logps.detach()
        metric_bag.get(Mean, prefix + "avg_chosen_logps").update(
            avg_chosen_logps.sum() / batch.chosen.batch_size,
            weight=batch.chosen.batch_size * weight_factor,
        )
    if avg_rejected_logps is not None:
        avg_rejected_logps = avg_rejected_logps.detach()
        metric_bag.get(Mean, prefix + "avg_rejected_logps").update(
            avg_rejected_logps.sum() / batch.rejected.batch_size,
            weight=batch.rejected.batch_size * weight_factor,
        )
...
def update_sequence_length_metrics(
    ...
    :param batch:
        The batch processed by the model.
    """
    metric_bag.get(Mean, prefix + "chosen_lengths").update(
        Tensor([batch.chosen.num_target_elements / batch.chosen.batch_size]),
        weight=batch.chosen.batch_size * weight_factor,
    )
    metric_bag.get(Mean, prefix + "rejected_lengths").update(
