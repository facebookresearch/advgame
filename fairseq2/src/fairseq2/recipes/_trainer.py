...
from typing import Any, Dict, Final, Generic, List, Mapping, TypeVar, final
...
class TrainUnit(ABC, Generic[BatchT_contra]):
    ...
    @property
    @abstractmethod
    def ema_model_attacker(self) -> Model: ...

    @property
    @abstractmethod
    def ema_model_defender(self) -> Model: ...

    @property
    @abstractmethod
    def ref_model_attacker(self) -> Model: ...

    @property
    @abstractmethod
    def ref_model_defender(self) -> Model: ...
...
class Trainer(Recipe):
    ...
    _dtype: DataType
    _amp: bool
    _optimizer_attacker: Optimizer
    _optimizer_defender: Optimizer
    _lr_scheduler_attacker: LRScheduler
    _lr_scheduler_defender: LRScheduler
    _loss_scaler_attacker: DynamicLossScaler
    _loss_scaler_defender: DynamicLossScaler
    _no_sync_grad_accumulation: bool
    _max_grad_norm: float | None
    _grad_check: bool
    _anomaly_detection: bool
    _train_attacker: bool
    _train_defender: bool
    _ema_attacker_ema_decay: float | None
    _ema_defender_ema_decay: float | None
    _ref_attacker_ema_decay: float | None
    _ref_defender_ema_decay: float | None
    _summon_full_parameters_for_validation: bool = True
    _rng_bag: RngBag
    _seed: int
    ...
    _checkpoint_manager_attacker: CheckpointManager
    _checkpoint_manager_defender: CheckpointManager
    _checkpoint_manager_ema_attacker: CheckpointManager
    _checkpoint_manager_ema_defender: CheckpointManager
    _checkpoint_manager_ref_attacker: CheckpointManager
    _checkpoint_manager_ref_defender: CheckpointManager
    ...
    _checkpoint_models: bool
    _checkpoint_ema_models: bool
    _checkpoint_ref_models: bool
    _save_model_only: bool
    _hugging_face_saver_attacker: HuggingFaceSaver | None
    _hugging_face_saver_defender: HuggingFaceSaver | None
    _hugging_face_saver_ema_attacker: HuggingFaceSaver | None
    _hugging_face_saver_ema_defender: HuggingFaceSaver | None
    _hugging_face_saver_ref_attacker: HuggingFaceSaver | None
    _hugging_face_saver_ref_defender: HuggingFaceSaver | None
    ...
    _first_iter: bool
    _batches: Sequence[List[Any]] | None
    _stop_requested: bool
    _num_batches_read: int
    _last_lr_attacker: float
    _last_lr_defender: float
    ...
    def __init__(
        ...
        *,
        ...
        train_attacker: bool = True,
        train_defender: bool = True,
        ema_attacker_ema_decay: float | None = None,
        ema_defender_ema_decay: float | None = None,
        ref_attacker_ema_decay: float | None = None,
        ref_defender_ema_decay: float | None = None,
        summon_full_parameters_for_validation: bool = True,
        ...
        checkpoint_ema_models: bool = True,
        checkpoint_ref_models: bool = True,
        save_model_only: bool = False,
        hugging_face_saver_attacker: HuggingFaceSaver | None = None,
        hugging_face_saver_defender: HuggingFaceSaver | None = None,
        hugging_face_saver_ema_attacker: HuggingFaceSaver | None = None,
        hugging_face_saver_ema_defender: HuggingFaceSaver | None = None,
        hugging_face_saver_ref_attacker: HuggingFaceSaver | None = None,
        hugging_face_saver_ref_defender: HuggingFaceSaver | None = None,
        ...
        self._optimizer_attacker = optimizer_attacker
        self._optimizer_defender = optimizer_defender

        self._lr_scheduler_attacker = lr_scheduler_attacker
        self._lr_scheduler_defender = lr_scheduler_defender

        fp16_init_scale, fp16_min_scale = fp16_loss_scale

        num_batches_accumulate_attacker = sum(
            [
                dr.num_accumulate
                for dr in data_readers
                if not dr._options.extras["skip_attacker"]
            ]
        )
        num_batches_accumulate_defender = sum(
            [dr.num_accumulate for dr in data_readers]
        )

        loss_scaler_attacker = DynamicLossScaler(
            optimizer_attacker,
            gangs.root,
            sharded=gangs.root.size != gangs.rdp.size,
            init_scale=fp16_init_scale,
            min_scale=fp16_min_scale,
            grad_accumulation=num_batches_accumulate_attacker,
            enabled=dtype == torch.float16,
        )
        loss_scaler_defender = DynamicLossScaler(
            optimizer_defender,
            gangs.root,
            sharded=gangs.root.size != gangs.rdp.size,
            init_scale=fp16_init_scale,
            min_scale=fp16_min_scale,
            grad_accumulation=num_batches_accumulate_defender,
            enabled=dtype == torch.float16,
        )

        assert loss_scaler_attacker.is_enabled == loss_scaler_defender.is_enabled

        self._loss_scaler_attacker = loss_scaler_attacker
        self._loss_scaler_defender = loss_scaler_defender
        ...
        self._train_attacker = train_attacker
        self._train_defender = train_defender
        self._ema_attacker_ema_decay = ema_attacker_ema_decay
        self._ema_defender_ema_decay = ema_defender_ema_decay
        self._ref_attacker_ema_decay = ref_attacker_ema_decay
        self._ref_defender_ema_decay = ref_defender_ema_decay
        self._summon_full_parameters_for_validation = (
            summon_full_parameters_for_validation
        )
        self._checkpoint_models = checkpoint_models
        self._checkpoint_ema_models = checkpoint_ema_models
        self._checkpoint_ref_models = checkpoint_ref_models
        ...
        self._checkpoint_manager_attacker = checkpoint_manager_attacker
        self._checkpoint_manager_defender = checkpoint_manager_defender
        self._checkpoint_manager_ema_attacker = checkpoint_manager_ema_attacker
        self._checkpoint_manager_ema_defender = checkpoint_manager_ema_defender
        self._checkpoint_manager_ref_attacker = checkpoint_manager_ref_attacker
        self._checkpoint_manager_ref_defender = checkpoint_manager_ref_defender
        ...
        self._hugging_face_saver_attacker = hugging_face_saver_attacker
        self._hugging_face_saver_defender = hugging_face_saver_defender
        self._hugging_face_saver_ema_attacker = hugging_face_saver_ema_attacker
        self._hugging_face_saver_ema_defender = hugging_face_saver_ema_defender
        self._hugging_face_saver_ref_attacker = hugging_face_saver_ref_attacker
        self._hugging_face_saver_ref_defender = hugging_face_saver_ref_defender
        ...
        self._last_lr_attacker = 0.0
        self._last_lr_defender = 0.0
    ...
    def _maybe_restore_state(self) -> _TrainerState:
        try:
            step_nr = self._checkpoint_manager_attacker.maybe_get_last_step_number(
    ...
    def _do_run(self) -> None:
        self._unit.model_attacker.module.train()
        self._unit.model_defender.module.train()
    ...
    def _read_next_batches(self) -> _TrainerState:
        with self._data_watch:
            try:
                self._batches_list = [next(dr) for dr in self._data_readers]
            ...
            if self._batches_list is not None:
                return _TrainerState.STEP

            for dr in self._data_readers:
                dr.reset()
    ...
    def _run_step(self, progress_task: ProgressTask) -> _TrainerState:
        try:
            self._checkpoint_manager_attacker.maybe_complete_async_checkpoint()
            self._checkpoint_manager_defender.maybe_complete_async_checkpoint()
            self._checkpoint_manager_ema_attacker.maybe_complete_async_checkpoint()
            self._checkpoint_manager_ema_defender.maybe_complete_async_checkpoint()
            self._checkpoint_manager_ref_attacker.maybe_complete_async_checkpoint()
            self._checkpoint_manager_ref_defender.maybe_complete_async_checkpoint()
        ...
        assert len(self._batches_list) == len(self._data_readers)

        for [batches, dr] in zip(self._batches_list, self._data_readers):
            with detect_anomaly:
                with record_function(f"step_{self._step_nr}"):
                    state = self._do_run_step(
                        progress_task,
                        prefix=f"{dr.split}/",
                        batches=batches,
                        scale=dr._options.extras["dataset_scale"],
                        skip_attacker=dr._options.extras["skip_attacker"],
                    )
        ...
        if self._ema_attacker_ema_decay is not None:
            log.info(
                f"Updating attacker ema model EMA with decay={self._ema_attacker_ema_decay}."
            )
            self._update_ema_model(
                ema_model=self._unit.ema_model_attacker,
                model=self._unit.model_attacker,
                decay=self._ema_attacker_ema_decay,
            )
        if self._ema_defender_ema_decay is not None:
            log.info(
                f"Updating defender ema model EMA with decay={self._ema_defender_ema_decay}."
            )
            self._update_ema_model(
                ema_model=self._unit.ema_model_defender,
                model=self._unit.model_defender,
                decay=self._ema_defender_ema_decay,
            )
        if self._ref_attacker_ema_decay is not None:
            log.info(
                f"Updating reference attacker model EMA with decay={self._ref_attacker_ema_decay}."
            )
            self._update_ema_model(
                ema_model=self._unit.ref_model_attacker,
                model=self._unit.model_attacker,
                decay=self._ref_attacker_ema_decay,
            )
        if self._ref_defender_ema_decay is not None:
            log.info(
                f"Updating reference defender model EMA with decay={self._ref_defender_ema_decay}."
            )
            self._update_ema_model(
                ema_model=self._unit.ref_model_defender,
                model=self._unit.model_defender,
                decay=self._ref_defender_ema_decay,
            )
    ...
    def _do_run_step(
        ...
        if self._loss_scaler_attacker.is_enabled:
            self._metric_bag.begin_updates()

        gangs = self._gangs

        num_targets_attacker = 0
        num_targets_defender = 0
        ...
        with self._compute_watch:
            ...
            for batch_nr in range(num_batches):
                ...
                try:
                    ...
                    with self._maybe_no_sync(
                        batch_nr, num_batches, self._unit.model_attacker
                    ):
                        with self._maybe_no_sync(
                            batch_nr, num_batches, self._unit.model_defender
                        ):
                            with record_function(f"step_{step_nr}_{batch_nr}_forward"):
                                (
                                    loss_attacker,
                                    loss_defender,
                                    num_batch_targets_attacker,
                                    num_batch_targets_defender,
                                ) = self._compute_loss(
                                    batch, prefix=prefix, skip_attacker=skip_attacker
                                )
                            ...
                            if num_batch_targets_attacker is None:
                                loss_attacker = loss_attacker / num_batches
                            else:
                                num_targets_attacker += num_batch_targets_attacker

                            if num_batch_targets_defender is None:
                                loss_defender = loss_defender / num_batches
                            else:
                                num_targets_defender += num_batch_targets_defender

                            with record_function(f"step_{step_nr}_{batch_nr}_backward"):
                                if self._train_attacker and not skip_attacker:
                                    self._loss_scaler_attacker.backward(
                                        loss_attacker * scale
                                    )
                                if self._train_defender:
                                    self._loss_scaler_defender.backward(
                                        loss_defender * scale
                                    )
            ...
            # the backward pass.
            total_num_targets_attacker = all_sum(gangs.dp, num_batch_targets_attacker)
            total_num_targets_defender = all_sum(gangs.dp, num_batch_targets_defender)
            if total_num_targets_attacker > 0:
                normalize_grads(
                    self._unit.model_attacker.module, gangs.dp, num_targets_attacker
                )
            if total_num_targets_defender > 0:
                normalize_grads(
                    self._unit.model_defender.module, gangs.dp, num_targets_defender
                )

            self._loss_scaler_attacker.unscale_grads_()
            self._loss_scaler_defender.unscale_grads_()

            # Clip the gradients.
            with record_function(f"step_{step_nr}_grad_norm"):
                grad_norm_attacker = self._unit.model_attacker.clip_grad_norm(
                    self._max_grad_norm
                )
                self._metric_bag.get(Mean, f"{prefix}grad_norm_attacker").update(
                    grad_norm_attacker
                )
                grad_norm_defender = self._unit.model_defender.clip_grad_norm(
                    self._max_grad_norm
                )
                self._metric_bag.get(Mean, f"{prefix}grad_norm_defender").update(
                    grad_norm_defender
                )

                if self._grad_check:
                    if not check_grad_norms(grad_norm_attacker, gangs.dp, step_nr):
                        raise InconsistentGradNormError(step_nr)
                    if not check_grad_norms(grad_norm_defender, gangs.dp, step_nr):
                        raise InconsistentGradNormError(step_nr)

            # Update the parameters.
            with record_function(f"step_{step_nr}_optimizer"):
                _, scale_result_attacker = (
                    self._loss_scaler_attacker.run_optimizer_step(step_nr)
                )
                _, scale_result_defender = (
                    self._loss_scaler_defender.run_optimizer_step(step_nr)
                )

            self._optimizer_attacker.zero_grad(set_to_none=True)
            self._optimizer_defender.zero_grad(set_to_none=True)

        if scale_result_attacker.overflow:
            if scale_result_attacker.min_reached:
        ...
        if scale_result_defender.overflow:
            if scale_result_defender.min_reached:
                raise MinimumLossScaleReachedError(step_nr)
        ...
        self._last_lr_attacker = get_effective_lr(self._lr_scheduler_attacker)
        self._last_lr_defender = get_effective_lr(self._lr_scheduler_defender)

        self._lr_scheduler_attacker.step()
        self._lr_scheduler_defender.step()
    ...
    def _update_ema_model(
        ...
        if ema_model.name != model.name:
            raise ValueError

        with torch.no_grad():
            state_dict_1 = ema_model.state_dict()
            state_dict_2 = model.state_dict()

            # check that the set of keys is the same
            ...
            for key in state_dict_1.keys():
                ...
                if isinstance(param1, torch.Tensor) and isinstance(
                    param2, torch.Tensor
                ):
                    new_param1 = param1 * decay + param2 * (1 - decay)
                    new_ema_state_dict[key] = new_param1.detach().clone()
                    # log.info(f"Mixed parameter {key} with shape {param1.shape}")
                else:
                    log.info(
                        f"Warning: Skipping mixing for non-tensor parameter {key} of type {type(param1)}"
                    )
                    new_ema_state_dict[key] = param1

            ema_model.load_state_dict(new_ema_state_dict)

        freeze_parameters(ema_model.module)

    def _maybe_no_sync(
        self, batch_nr: int, num_batches: int, model: Model
    ) -> ContextManager:
    ...
    def _compute_loss(
        self, batch: Any, prefix: str, skip_attacker: bool
    ...
    def _checkpoint(self, blocking: bool) -> None:
        if self._checkpoint_models:
            self._checkpoint_custom(
                blocking,
                self._checkpoint_manager_attacker,
                self._unit.model_attacker,
                self._hugging_face_saver_attacker,
            )
            self._checkpoint_custom(
                blocking,
                self._checkpoint_manager_defender,
                self._unit.model_defender,
                self._hugging_face_saver_defender,
            )
        if self._checkpoint_ema_models:
            self._checkpoint_custom(
                blocking,
                self._checkpoint_manager_ema_attacker,
                self._unit.ema_model_attacker,
                self._hugging_face_saver_ema_attacker,
            )
            self._checkpoint_custom(
                blocking,
                self._checkpoint_manager_ema_defender,
                self._unit.ema_model_defender,
                self._hugging_face_saver_ema_defender,
            )
        if self._checkpoint_ref_models:
            self._checkpoint_custom(
                blocking,
                self._checkpoint_manager_ref_attacker,
                self._unit.ref_model_attacker,
                self._hugging_face_saver_ref_attacker,
            )
            self._checkpoint_custom(
                blocking,
                self._checkpoint_manager_ref_defender,
                self._unit.ref_model_defender,
                self._hugging_face_saver_ref_defender,
            )

    def _checkpoint_custom(
        self,
        blocking: bool,
        checkpoint_manager: CheckpointManager,
        model: Model,
        hg_saver: HuggingFaceSaver | None = None,
    ) -> None:
        step_nr = self._step_nr

        log.info("Preparing checkpoint at step {}.", step_nr)

        self._maybe_wait_checkpoint_custom(checkpoint_manager)
        ...
        try:
            if self._save_model_only:
                ...
                def callback(step_nr: int, blocking: bool) -> None:
                    self._complete_checkpoint(
                        step_nr, blocking, checkpoint_manager, hg_saver
                    )

                checkpoint_manager.save_model_only(
                    step_nr,
                    model,
    ...
    def _complete_checkpoint(
        ...
        self._delete_stale_checkpoints(checkpoint_manager)
    ...
    def _publish_metrics(self) -> None:
        ...
        if gangs.root.rank == 0:
            ...
            values["lr_attacker"] = self._last_lr_attacker
            values["lr_defender"] = self._last_lr_defender
    ...
    def _validate(self) -> float | None:
        ...
        if self._summon_full_parameters_for_validation:
            with self._unit.model_attacker.summon_full_parameters():
                with self._unit.model_defender.summon_full_parameters():
        ...
        if score is not None:
            if self._should_checkpoint():
                self._save_score(score, self._checkpoint_manager_attacker)
                self._save_score(score, self._checkpoint_manager_defender)
                self._save_score(score, self._checkpoint_manager_ema_attacker)
                self._save_score(score, self._checkpoint_manager_ema_defender)
                self._save_score(score, self._checkpoint_manager_ref_attacker)
                self._save_score(score, self._checkpoint_manager_ref_defender)
    ...
    def _save_score(self, score: float, checkpoint_manager: CheckpointManager) -> None:
        try:
            checkpoint_manager.save_score(self._step_nr, score)
        except CheckpointSaveError as ex:
            raise RecipeError(
    ...
    def _maybe_wait_checkpoint(self) -> None:
        self._maybe_wait_checkpoint_custom(self._checkpoint_manager_attacker)
        self._maybe_wait_checkpoint_custom(self._checkpoint_manager_defender)
        self._maybe_wait_checkpoint_custom(self._checkpoint_manager_ema_attacker)
        self._maybe_wait_checkpoint_custom(self._checkpoint_manager_ema_defender)
        self._maybe_wait_checkpoint_custom(self._checkpoint_manager_ref_attacker)
        self._maybe_wait_checkpoint_custom(self._checkpoint_manager_ref_defender)

    def _maybe_wait_checkpoint_custom(
        self, checkpoint_manager: CheckpointManager
    ) -> None:
        if not checkpoint_manager.is_saving:
            return

        log.info("Waiting for the current checkpoint save operation to complete before continuing.")  # fmt: skip

        try:
            checkpoint_manager.maybe_complete_async_checkpoint(blocking=True)
        except CheckpointSaveError as ex:
            raise RecipeError(
                f"The checkpoint of step {ex.step_nr} cannot be saved. See the nested exception for details."
            ) from ex

    def _delete_stale_checkpoints(self, checkpoint_manager: CheckpointManager) -> None:
        try:
            stale_step_nrs = checkpoint_manager.get_stale_step_numbers(
        ...
        for step_nr in stale_step_nrs:
            try:
                checkpoint_manager.delete_checkpoint(step_nr)
    ...
    @override
    def close(self) -> None:
        self._checkpoint_manager_attacker.close()
        self._checkpoint_manager_defender.close()
        self._checkpoint_manager_ema_attacker.close()
        self._checkpoint_manager_ema_defender.close()
        self._checkpoint_manager_ref_attacker.close()
        self._checkpoint_manager_ref_defender.close()
