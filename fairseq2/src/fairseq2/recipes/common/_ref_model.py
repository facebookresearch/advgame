...
def setup_reference_model(
    ...
    model = setup_data_parallel_model(
        context, trainer_section, model, gangs, has_checkpoint=False, static_graph=True
    )
...
def load_reference_model(
    ...
    try:
        ...
        raise RecipeError(
            f"The '{ex.model_name}' reference model cannot be loaded. See the nested exception for details."
        ) from ex
    ...
    def load(
        ...
        try:
            ...
            raise ModelLoadError(
                model_name, f"The '{model_name}' reference model configuration cannot be loaded. See the nested exception for details."  # fmt: skip
            ) from ex

        # Load the model.
        log.info("Loading '{}' reference model on data parallel rank 0.", model_name)
        ...
        try:
            ...
            raise ModelLoadError(
                model_name, f"The '{model_name}' reference model cannot be constructed due to an unsupported operation. See the nested exception for details."  # fmt: skip
            ) from ex
        ...
        try:
            ...
            raise ModelLoadError(
                model_name, f"The collective barrier after the '{model_name}' reference model load operation has failed. See the nested exception for details."  # fmt: skip
            ) from ex
