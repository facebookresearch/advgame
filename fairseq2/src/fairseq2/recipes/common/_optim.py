...
def create_optimizer_from_params(
    context: RuntimeContext, optimizer_section: OptimizerSection, params
) -> Optimizer:
    optimizer_handlers = context.get_registry(OptimizerHandler)

    creator = _OptimizerCreatorFromParams(optimizer_handlers)

    return creator.create(optimizer_section, params)


@final
class _OptimizerCreatorFromParams:
    _optimizer_handlers: Provider[OptimizerHandler]

    def __init__(self, optimizer_handlers: Provider[OptimizerHandler]) -> None:
        self._optimizer_handlers = optimizer_handlers

    def create(self, optimizer_section: OptimizerSection, params) -> Optimizer:
        try:
            handler = self._optimizer_handlers.get(optimizer_section.name)
        except LookupError:
            raise UnknownOptimizerError(optimizer_section.name) from None
