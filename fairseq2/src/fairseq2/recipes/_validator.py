...
from typing import Any, List, TypeVar, final
...
class Validator:
    ...
    def _maybe_restore_best_score_and_step(self) -> None:
        ...
        try:
            if self._gangs.root.rank == 0:
                scores_and_steps_attacker = (
                    self._checkpoint_manager_attacker.load_scores()
                )
                scores_and_steps_defender = (
                    self._checkpoint_manager_defender.load_scores()
                )
                assert scores_and_steps_attacker == scores_and_steps_defender
                scores_and_steps = scores_and_steps_attacker
            else:
                scores_and_steps = []
    ...
    def _do_run(self, train_step_nr: int) -> float | None:
        ...
        assert len(self._units) == len(self._data_readers)

        for unit, data_reader in zip(self._units, self._data_readers):
            if unit.name:
                log.info(
                    f"Validating {unit.name} (skip_attacker={data_reader._options.extras['skip_attacker']})."
                )
    ...
    def _run_unit(
        ...
        train_step_nr: int,
        unit: EvalUnit[Any],
        data_reader: DataReader[Any],
    ) -> float | None:
        unit.model_attacker.module.eval()
        unit.model_defender.module.eval()
            ...
            while not eod:
                ...
                with record_function(f"step_{self._step_nr}"):
                    self._run_step(
                        ...
                        batches,
                        metric_bag,
                        prefix=f"{data_reader.split}/",
                        skip_attacker=data_reader._options.extras["skip_attacker"],
                    )
