...
from fairseq2.recipes.lm import (  # GroupDpoFinetuneUnitHandler,
    ...
    GenerativeGameVerifierHandler,
    GenerativePairwiseVerifierHandler,
    GenerativePointwiseVerifierHandler,
    GrpoFinetuneUnitHandler,
    GrpoGameFinetuneUnitHandler,
    GSM8kVerifierHandler,
    J1PairwiseScoreExtractorHandler,
    J1PointwiseExtractorHandler,
    JudgmentExtractorHandler,
    AttackerPairwiseScoreExtractorHandler,
    DefenderPairwiseScoreExtractorHandler,
    AttackerPointwiseScoreExtractorHandler,
    DefenderPointwiseScoreExtractorHandler,
    PromptMaskPointwiseExtractorHandler,
    MathVerifyHandler,
    NoEnvAtheneRewardPipeline,
    NoEnvGeneralVerifierPipeline,
    OnlineDpoFinetuneUnitHandler,
    OnlineDpoGameFinetuneUnitHandler,
...
def _register_online_finetune_units(context: RuntimeContext) -> None:
    ...
    # Online DPO game
    handler = OnlineDpoGameFinetuneUnitHandler(context)
    registry.register(handler.name, handler)
    ...
    # GRPO Game
    handler = GrpoGameFinetuneUnitHandler(context)
    registry.register(handler.name, handler)
    ...
    registry = context.get_registry(VLLMOutputRewardGameHandler)

    # GenerativeGameVerifier
    handler = GenerativeGameVerifierHandler()
    registry.register(handler.name, handler)
    ...
    handler = AttackerPairwiseScoreExtractorHandler()
    registry.register(handler.name, handler)

    handler = DefenderPairwiseScoreExtractorHandler()
    registry.register(handler.name, handler)

    handler = AttackerPointwiseScoreExtractorHandler()
    registry.register(handler.name, handler)

    handler = DefenderPointwiseScoreExtractorHandler()
    registry.register(handler.name, handler)

    handler = PromptMaskPointwiseExtractorHandler()
    registry.register(handler.name, handler)
